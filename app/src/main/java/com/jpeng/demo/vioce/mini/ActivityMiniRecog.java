...
public class ActivityMiniRecog extends AppCompatActivity implements EventListener {
    ...
    public void onEvent(String name, String params, byte[] data, int offset, int length) {
        ...
        if (params != null && !params.isEmpty()) {
            if (name.equals("asr.partial")){
                Gson gson=new Gson();
                recong=gson.fromJson(params,Recong.class);
            }
            if (name.equals("asr.finish")){
                txtResult.setText(recong.getBest_result()+"耗时："+String.valueOf(System.currentTimeMillis()-time)+"毫秒");
            }
    ...
    public void getNetInfor() {
        ...
        if (info != null && info.isConnected()) {
            Toast.makeText(ActivityMiniRecog.this, "网络已连接", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(ActivityMiniRecog.this, "网络连接失败", Toast.LENGTH_SHORT).show();
            isInfo=false;
        }
    }
